...
def sort_by_lengths(word_lists, tag_lists):
    word_lists, indices = sort_word_lists(word_lists)
    tag_lists = [tag_lists[i] for i in indices]

    return word_lists, tag_lists, indices


def sort_word_lists(word_lists):
...
def cal_weighted_loss(logits, targets, tag2id, alpha=TrainingConfig.alpha):
    """计算损失
    参数:
        logits: [B, L, out_size]
        targets: [B, L]
        lengths: [B]
        alpha: 超参数，alpha越大，实体标记的影响越大
    """
    PAD = tag2id.get('<pad>')
    assert PAD is not None
    ...
    O_tagids = [id_ for tag, id_ in tag2id.items() if tag.endswith('O')]

    weights = torch.ones(len(tag2id), device=logits.device) * alpha
    for O_tagid in O_tagids:
        weights[O_tagid] = 1.
...
def time_format(seconds):
    ...
    seconds = round(seconds - hour*3600 - mins*60)
    return "{}小时{}分钟{}秒".format(hour, mins, seconds)


def get_optimizer(model, embedding_lr, other_lr):
    """构建优化器"""
    ...
    for name, param in model.named_parameters():
        if name.endswith("embedding.weight"):
            params_list.append(
                dict(params=param, lr=embedding_lr)
            )
        else:
            params_list.append(
                dict(params=param, lr=other_lr)
            )
...
def change_embedding_lr(optimizer, embedding_lr):
